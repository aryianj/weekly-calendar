...
function App() {
  ...
  return (
    <>
      ...
      {(isMobile) && (
        <div className="flex h-screen w-screen bg-gradient-to-b from-cyan-200 to-sky-50 items-center justify-center">
        <div className="flex flex-col justify-center bg-white max-w-full sm:max-w-[360px] w-full sm:h-[500px] h-full rounded-3xl border-1 border-cyan-600 p-4">
          <button className="self-center sm:ml-32 ml-0 border-1 rounded-3xl px-3 py-1 h-fit w-fit" onClick={() => setTimeZone(timeZone === "PST" ? "EST" : "PST")}>
            {timeZone}
          </button>
          ...
          <div className="flex flex-col justify-center items-center mt-4">
            <div className="grid grid-cols-1 sm:grid-cols-7 gap-4 sm:gap-6 w-full">
              {['SUNDAY', 'MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY'].map((day, index) => (
                <div key={index} className="border-1 p-3 rounded-3xl">
                  <h1 className="text-center text-xl">{day}</h1>
                  <ul className="list-disc pl-1">
                    <li className="flex items-center gap-2">
                      <span className="w-3 h-3 border-1 border-black bg-red-400 rounded-full"></span>
                      Work
                  ...
                  <p className="text-sm pt-2">{index + 16}</p>
                </div>
              ))}
            </div>
